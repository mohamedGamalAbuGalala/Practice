...
vector<vi> adj;
vi vsNum, prnt, low, S, vs, SCC, root;
...
void tarjanSCC(int u) {
	...
	if (low[u] == vsNum[u]) {
		int v;
		while (1) {
			v = S.back(), S.pop_back(), vs[v] = 0, SCC[v] = numSCC;
			if (u == v)
				break;
		}
		numSCC++;
...
int main() {
	...
	while (T--) {
		...
		for (int i = 0; i < E and in(u) and in(v); ++i)
			adj[u].pb(v);
		SCC.assign(V + 1, 0), vsNum.assign(V + 1, WH), low.assign(V + 1, 0), vs.assign(
				V + 1, 0), S.clear(), prnt.clear();
		dfsNumberCounter = numSCC = roots = 0;
		for (int i = 1; i <= V; i++)
			if (vsNum[i] == WH)
				tarjanSCC(i);
